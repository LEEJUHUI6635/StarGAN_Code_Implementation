...
class Solver(object):
    def __init__(self, config, data_loader):
        ...
        # For visualization -> debugging 성공, 문제 x
        ...
        self.logger_iter = config.logger_iter # 1000
    ...
    def train(self):
        ...
        for iters in range(start_iters, self.nb_iters):
            ...
            real_src_loss = -torch.mean(out_src) # A, mean 값을 최대화 -> real image를 real로 인식해야 하기 때문
            ...
            real_cls_loss = F.binary_cross_entropy_with_logits(out_cls, labels, size_average=False) / self.batch_size
            # real_cls_loss2 = self.criterion(out_cls, labels) # B
            # real_cls_loss = F.binary_cross_entropy_with_logits(out_cls, labels) / self.batch_size
            # print(real_cls_loss, real_cls_loss2)
            ...
            fake_src_loss = torch.mean(fake_out_src) # C, mean 값을 최소화 -> fake image를 fake로 인식해야 하기 때문
            ...
            if iters % 5 == 0:
                ...
                G_src_loss = -torch.mean(G_out_src) # A, mean 값을 최대화 -> fake image를 real로 인식해야 하기 때문
                # G_cls_loss = self.criterion(G_out_cls, trg_labels) / self.batch_size
                # G_cls_loss = self.criterion(G_out_cls, trg_labels) # B
                G_cls_loss = F.binary_cross_entropy_with_logits(G_out_cls, trg_labels, size_average=False) / self.batch_size
